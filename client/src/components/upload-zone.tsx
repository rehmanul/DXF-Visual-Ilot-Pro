...
{
  onUploadComplete: ( data: { floorPlanId: number; basePlanData: any } ) => void;
}
...
export default function UploadZone ( { onUploadComplete }: UploadZoneProps )
{
  ...
  const uploadMutation = useMutation( {
    mutationFn: async ( file: File ) =>
    {
      ...
      const response = await fetch( '/api/extract-base-plan', {
        method: 'POST',
        body: formData,
      } );
    ...
    onSuccess: ( data: any, file: any ) =>
    {
      toast( {
        ...
        description: `File ${ file.name } is now being processed.`,
      } );
    ...
    onError: ( error: any, file: any ) =>
    {
      toast( {
        title: "Upload failed",
        description: `For ${ file.name }: ${ error.message }`,
        variant: "destructive",
      } );
      setUploadedFiles( prev => prev.filter( f => f !== file ) );
    }
    ...
    fileArray.forEach( file =>
    {
      ...
      if ( !allowedTypes.includes( ext ) )
      {
        toast( {
          ...
          description: "Please upload DXF, DWG, PDF, or image files (JPG, PNG) only.",
          variant: "destructive",
        } );
        return;
      }
      ...
      {
        toast( {
          title: "CAD file detected",
          description: "Processing CAD file for optimal results.",
        } );
      } else if ( ext === '.pdf' )
      {
        toast( {
          title: "PDF file detected",
          description: "Extracting vector data from PDF. This may take longer.",
        } );
      } else if ( [ '.jpg', '.jpeg', '.png' ].includes( ext ) )
      {
        toast( {
          title: "Image file detected",
          description: "Converting image to floor plan. Results may vary based on image quality.",
        } );
      }
  ...
  const handleClick = () =>
  {
    ...
    input.id = 'dxf-upload-input';
    input.type = 'file';
    input.accept = '.dxf,.dwg,.pdf,.jpg,.jpeg,.png';
    input.multiple = true;
    input.onchange = ( e ) =>
    {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-500 mb-4">
          Drag and drop your CAD files or floor plan images here, or click to browse
        </p>
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <ul className="text-sm text-blue-700 space-y-1">
              ...
              <li>• <strong>JPG/PNG files:</strong> Floor plan images or scans</li>
            </ul>
