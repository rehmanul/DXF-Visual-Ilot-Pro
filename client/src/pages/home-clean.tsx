...
import { ProcessingResult, Room } from "@shared/schema";
...
export default function Home ()
{
  ...
  useEffect( () =>
  {
    if ( floorPlanData?.floorPlan )
    {
      ...
      {
        setCurrentStep( 3 );
        if ( floorPlanData.floorPlan.geometryData )
        {
          setBasePlanData( floorPlanData.floorPlan.geometryData );
        }
        if ( floorPlanData.floorPlan.ilotLayout )
        {
          setLayout( floorPlanData.floorPlan.ilotLayout );
          setVisualizationOptions( prev => ( { ...prev, mode: 'detailed' } ) );
        }
  ...
  const handleUploadComplete = ( data: { floorPlanId: number, basePlanData: any } ) =>
  {
    ...
    setCurrentStep( 3 );
    setVisualizationOptions( prev => ( { ...prev, mode: 'base' } ) );
  };
  ...
  return (
    <ResponsiveLayout currentStep={ currentStep } onStepChange={ setCurrentStep }>
      <div className="space-y-6">
        ...
        { currentStep === 3 && selectedFloorPlanId && ( basePlanData || floorPlanData ) && (
          <Card>
            <CardHeader>
              ...
              <div className="h-[60vh] sm:h-[70vh] lg:h-[80vh] rounded-lg overflow-hidden border border-gray-200 bg-white">
                { viewMode === '2d' ? (
                  <AdvancedFloorPlanRenderer
                    basePlanData={ basePlanData || floorPlanData?.floorPlan?.geometryData }
                    layout={ layout || floorPlanData?.floorPlan?.ilotLayout }
                    options={ visualizationOptions }
                  />
                ) : (
                  <FloorPlan3DViewer
                    floorPlan={ floorPlanData?.floorPlan }
                    rooms={ floorPlanData?.rooms as Room[] || [] }
        ...
        { currentStep === 4 && selectedFloorPlanId && floorPlanData && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                <ExportPanel floorPlan={ floorPlanData.floorPlan } />
              </CardContent>
            </Card>
